...
namespace Leita\Resl;
	...
	function separate($args){
		$kids = array_filter($args, 'is_array');
		$properties = array_diff_key($args, $kids);
		$style = preg_filter('/^_(.*\=.*)/', "$1", $properties);
		$properties = array_diff_key($properties, $style);
		$attr = preg_grep('/^.*\=.*/', $properties);
		$properties = array_diff_key($properties, $attr);
		$kids = array_merge($properties, $kids);
		$attr = array_map(function($value){
			$split = explode('=', $value);
			return [$split[0] => $split[1]];
		}, $attr);
		$style = array_map(function($value){
			$split = explode('=', $value);
			return [$split[0] => $split[1]];
		}, $style);
		//echo "<pre>separate.args: ", htmlentities(print_r($args, true)), "</pre>";
		//echo "<pre>separate.attr: ", htmlentities(print_r($attr, true)), "</pre>";
		//echo "<pre>separate.style: ", htmlentities(print_r($style, true)), "</pre>";
		//echo "<pre>separate.kids: ", htmlentities(print_r($kids, true)), "</pre>";
		//return $args;
		return array_merge($attr, $style, $kids);
	}
	...
	function tags(...$args){
		return array_merge(['tagname' => "tags"], separate($args));
	};
	...
	function toHtml(array &$array){
		echo "<pre>array: ", htmlentities(print_r($array, true)), "</pre>";
		$tagname = NULL;
		$attr = []; 
		$style = []; 
		$kids = [];
		...
		//echo "<pre>tagname: ", htmlentities(print_r($tagname, true)), "</pre>";
		//echo "<pre>attr: ", htmlentities(print_r($attr, true)), "</pre>";
		//echo "<pre>style: ", htmlentities(print_r($style, true)), "</pre>";
		//echo "<pre>kids: ", htmlentities(print_r($kids, true)), "</pre>";
